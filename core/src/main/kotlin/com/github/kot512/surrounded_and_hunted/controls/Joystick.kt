...
class Joystick(
    ...
    init {
        setPosition(150f, 110f)
        this.width = GameScreen.LEVEL_HEIGHT / 4
        this.height = GameScreen.LEVEL_HEIGHT / 4
    ...
    private val knobPosition = Point(this.x + knobRadius, this.y + knobRadius)
    ...
    fun moveKnob(touchPointX: Float, touchPointY: Float) {
        var x = touchPointX - baseRadius
        var y = touchPointY - baseRadius

        val deviation = sqrt(x.pow(2) + y.pow(2))
        if (deviation > baseRadius) {
            x = x * baseRadius / deviation
            y = y * baseRadius / deviation
        }
        knobDeviation.setPoint(Point(x / baseRadius, y / baseRadius))
        knobPosition.setPoint(Point(x + knobRadius + baseRadius, y + knobRadius + baseRadius))
